...
export class TableManagementComponent implements OnInit {
  ...
  createNewSection(): void {
    // Generate next section letter (A, B, C, etc.) but stop at Z
    const currentSections = this.sections();
    const nextSectionIndex = currentSections.length;
    
    // Check if we've reached the maximum (Z = 25th letter, 0-indexed)
    ...
    const nextSection = String.fromCharCode(65 + nextSectionIndex); // A=65, B=66, etc.
    
    // Add new section to sections array
    const updatedSections = [...currentSections, nextSection];
    this.sections.set(updatedSections);
  ...
  resetTableForm(): void {
    ...
    this.isCreatingTable.set(false);
     ...
     try {
      ...
      if (response?.data) {
        this.tables.set([...this.tables(), response.data]);
      }
  ...
  async confirmDeleteTable(): Promise<void> {
    ...
    try {
      ...
      if (response?.success) {
        this.tables.set(this.tables().filter(t => t.table_id !== table.table_id));
